...
// use function when YouTube player changes state (e.g. video ends, etc.)
...
function clicked(e) {
    var target = e.target;
    // check if we clicked on td html element
    // and get data-id attribute of the parent tr element
    if (target.tagName === 'TD') {
        currentTrack = target.parentNode.dataset.id;
        youtubePlayer.loadVideoById(currentTrack, 0, 'default');
    }
}
...
searchForm.addEventListener('submit', function(e) {
    ...
    request.onreadystatechange = function() {
        if (request.readyState === 4 && request.status === 200) {
            ...
            // set current track to the first track in the array
            // currentTrack = playlist[0];

            // load the first track from the array
            // youtubePlayer.loadVideoById(currentTrack, 0, 'default');
        }
    }
